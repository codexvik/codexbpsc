...
from ingestion.state_store import connect as ingestion_connect, get_job_runs, get_recent as get_recent_seen
...
<aside class="sidebar">
  ...
  <nav class="nav-list">
    ...
    <a class="nav-item {{ 'active' if current == 'notices' else '' }}" href="/admin/notices">
      Notices
      {% if pending_count %}<span class="nav-badge">{{ pending_count }}</span>{% endif %}
    </a>
    <a class="nav-item {{ 'active' if current == 'extract' else '' }}" href="/admin/extract">Run Extraction</a>
...
<h2 class="section-heading">Crawler job status</h2>
<div class="job-status-row">
  {% for j in jobs %}
  <div class="job-status-card">
    <span class="job-status-dot {{ j.dot }}"></span>
...
<table class="notices-table" id="exams-table">
  ...
  <tbody>
  {% for e in exams %}
  <tr data-search="{{ (e.name ~ ' ' ~ (e.advt_no or '')) | lower }}">
...
<form class="extract-form" method="post" action="/admin/exams/{{ exam.id }}/extract" data-banner-text="Extracting — this can take up to 30 seconds per URL.">
  ...
  <div class="actions">
    <button class="approve" type="submit" data-busy-text="Extracting…">Extract &amp; Save</button>
  </div>
</form>
  ...
  <tr>
    ...
    <td class="col-actions">
      ...
      <form method="post" action="/admin/notify/{{ n.id }}" class="inline-form" data-banner-text="Sending to subscribers…">
        <input type="hidden" name="redirect_to" value="/admin/exams/{{ exam.id }}">
        <button class="send small" type="submit" data-busy-text="…">Send</button>
      </form>
      {% elif n.notified_at %}
...
<table class="notices-table">
  ...
  <tr>
    ...
    <td class="col-actions">
      {% if not n.reviewed and not n.rejected %}
      <form method="post" action="/admin/review/{{ n.id }}/approve" class="inline-form">
        <input type="hidden" name="status" value="{{ filters.status }}"><input type="hidden" name="source_id" value="{{ filters.source_id }}"><input type="hidden" name="confidence" value="{{ filters.confidence }}">
        <button class="approve small" type="submit" data-busy-text="…">Approve</button>
      </form>
      <form method="post" action="/admin/review/{{ n.id }}/reject" class="inline-form">
        <input type="hidden" name="status" value="{{ filters.status }}"><input type="hidden" name="source_id" value="{{ filters.source_id }}"><input type="hidden" name="confidence" value="{{ filters.confidence }}">
...
<p class="page-sub">
  The scheduler checks BPSC every 20 minutes and finds pages that changed — that part is
  automatic. Turning one of those into a real notice still takes you picking it and confirming
  it below. Notices that pass the confidence gate go live immediately; flagged ones land in
  <a href="/admin/notices?status=pending">Notices</a>.
</p>
...
<form class="card" method="post" action="/admin/extract/run" data-banner-text="Extracting — this can take up to 30 seconds per URL. Don't close this tab.">
  ...
  <div class="actions">
    <button class="approve" type="submit" data-busy-text="Extracting…">Extract Selected</button>
  </div>
</form>
<script>
...
def dashboard():
    with get_connection() as conn:
        ...
        kpis = conn.execute(
            ...
            SELECT count(*) FILTER (WHERE NOT reviewed AND NOT rejected) AS pending,
                   count(*) FILTER (WHERE reviewed) AS approved,
    ...
    jobs = [
        _job_status(job_runs, "notice_poll:bpsc_bihar", "BPSC Notice Crawl"),
        _job_status(job_runs, "calendar_sync:bpsc_bihar", "BPSC Exam Calendar Sync"),
    ]
...
def settings_page(saved: Optional[int] = Query(default=None)):
    with get_connection() as conn:
        pending_count = _pending_count(conn)
    ...
    for sid in active_source_ids():
        ...
        sources.append(cfg)
    body = _env.from_string(_SETTINGS).render(sources=sources, jobs=jobs, saved=bool(saved))
...
def exams_list():
    ...
    body = _env.from_string(_EXAMS_LIST).render(exams=rows, from_calendar_count=from_calendar_count)
...
def notices_page(
    ...
    filters = {"status": status, "source_id": source_id, "confidence": confidence}
    body = _env.from_string(_NOTICES_PAGE).render(
...
def approve(notice_id: int, status: str = Form(""), source_id: str = Form(""), confidence: str = Form(""), redirect_to: str = Form("")):
    with get_connection() as conn:
        conn.execute(
            "UPDATE notices SET reviewed = true, rejected = false, reviewed_at = now() WHERE id = %s",
            (notice_id,),
        )
    return RedirectResponse(url=redirect_to or _notices_redirect_url(status, source_id, confidence), status_code=303)


@router.post("/review/{notice_id}/reject")
def reject(notice_id: int, status: str = Form(""), source_id: str = Form(""), confidence: str = Form(""), redirect_to: str = Form("")):
    with get_connection() as conn:
        conn.execute(
...
def notify(notice_id: int, status: str = Form(""), source_id: str = Form(""), confidence: str = Form(""), redirect_to: str = Form("")):
...
def extract_form():
    with get_connection() as conn:
        pending_count = _pending_count(conn)
        candidates = _extraction_candidates(conn, "bpsc_bihar")
    body = _env.from_string(_EXTRACT_FORM).render(source_ids=active_source_ids(), candidates=candidates)
    return _render("Run Extraction", body, pending_count, current="extract")
...
def extract_run(source_id: str = Form("bpsc_bihar"), urls: str = Form(""), selected_urls: list[str] = Form(default=[])):
    ...
    return _render("Extraction Results", body, pending_count, current="extract")
